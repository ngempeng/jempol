...
echo -e "  Author : ${green} Erfan RInanda ® ${NC}${YELLOW}(${NC} ${green} ErfanRinanda ${NC}${YELLOW})${NC}"
echo -e " © Recode By My @erfanrinanda ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
function first_setup(){
    ...
    add-yum-repository ppa:vbernat/haproxy-2.0 -y
    yum -y install haproxy=2.0.\*
elif [[ $(cat /etc/os-release | grep -w ID | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/ID//g') == "debian" ]]; then
    ...
    echo deb "[signed-by=/usr/share/keyrings/haproxy.debian.net.gpg]" \
        ...
        >/etc/yum/sources.list.d/haproxy.list
    yum update
    yum -y install haproxy=1.8.\*
else
...
function nginx_install() {
    ...
    if [[ $(cat /etc/os-release | grep -w ID | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/ID//g') == "ubuntu" ]]; then
        ...
        # // sudo add-yum-repository ppa:nginx/stable -y 
        yum install nginx -y 
    elif [[ $(cat /etc/os-release | grep -w ID | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/ID//g') == "debian" ]]; then
...
function base_package() {
    ...
    yum install zip pwgen openssl netcat socat cron bash-completion -y
    ...
    yum remove --purge exim4 -y
    yum remove --purge ufw firewalld -y
    yum install -y --no-install-recommends software-properties-common
    echo iptables-persistent iptables-persistent/autosave_v4 boolean true | debconf-set-selections
    echo iptables-persistent iptables-persistent/autosave_v6 boolean true | debconf-set-selections
    yum install -y speedtest-cli vnstat libnss3-dev libnspr4-dev pkg-config libpam0g-dev libcap-ng-dev libcap-ng-utils libselinux1-dev libcurl4-nss-dev flex bison make libnss3-tools libevent-dev bc rsyslog dos2unix zlib1g-dev libssl-dev libsqlite3-dev sed dirmngr libxml-parser-perl build-essential gcc g++ python htop lsof tar wget curl ruby zip unzip p7zip-full python3-pip libc6 util-linux build-essential msmtp-mta ca-certificates bsd-mailx iptables iptables-persistent netfilter-persistent net-tools openssl ca-certificates gnupg gnupg2 ca-certificates lsb-release gcc shc make cmake git screen socat xz-utils yum-transport-https gnupg1 dnsutils cron bash-completion ntpdate chrony jq openvpn easy-rsa
...
# // Installing Dropbear
yum install dropbear -y #> /dev/null 2>&1
...
# setting vnstat
yum -y install vnstat > /dev/null 2>&1
/etc/init.d/vnstat restart
yum -y install libsqlite3-dev > /dev/null 2>&1
...
rm -rf wondershaper
echo > /home/limit
yum install msmtp-mta ca-certificates bsd-mailx -y
cat<<EOF>>/etc/msmtprc
defaults
tls on
tls_starttls on
